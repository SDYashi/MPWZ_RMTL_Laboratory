...
export class SolarGenMeterCertificatePdfService {
  ...
  private hdr(meta: { zone: string; method: string; status: string }) {
    return {
      margin: [40, 16, 40, 6],
      stack: [
        {
        ...
        { text: `DC/Zone: ${meta.zone || '-'}    •    Test Method: ${meta.method || '-'}    •    Test Status: ${meta.status || '-'}`, alignment: 'center', fontSize: 9, color: '#555', margin: [0,6,0,0] }
      ]
    };
  }

  private page(r: GenRow) {
    const section = { bold: true, fontSize: 12, margin: [0,10,0,6], decoration: 'underline' };
    const label = { bold: true, fillColor: '#f5f5f5', margin: [0,2,0,2] };
    const value = { margin: [0,2,0,2] };

    const blocks: any[] = [
      { text: 'SOLAR GENERATION METER TEST REPORT', alignment: 'center', bold: true, fontSize: 14, margin: [0,5,0,0] },
    ];
    if (r.certificate_no) {
    ...
    // Consumer & Meter
    blocks.push(
      { text: 'Consumer & Meter Information', ...section },
      {
        table: {
          widths: ['30%','70%'],
          body: [
  ...
  private buildDoc(header: GenHeader, rows: GenRow[]) {
    const meta = {
    ...
    if (data.length > 1) {
      content.push(
        { text: 'SOLAR GENERATION METER TEST REPORTS', alignment: 'center', bold: true, fontSize: 16, margin: [0, 140, 0, 10] },
  ...
  download(header: GenHeader, rows: GenRow[]) {
