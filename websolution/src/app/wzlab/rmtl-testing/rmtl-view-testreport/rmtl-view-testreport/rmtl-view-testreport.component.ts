...
export class RmtlViewTestreportComponent implements OnInit {
  ...
  async downloadTestreports_byreportidwithReportTypes(
    ...
    try {
      ...
      // NEW: your response is already an array of objects that can contain:
      // { testing, device, assignment, lab, user, testing_bench }
      const rowsRaw: Array<{
        testing: any;
        device?: any;
        assignment?: any;
      ...
      const commonHeaderBase = {
        ...
        lab_name: S(
          lab0.lab_pdfheader_name ||
          lab0.lab_name ||
          t0.lab_name ||
          ''
        ),
        lab_address: S(
          lab0.lab_pdfheader_address ||
          lab0.lab_location ||
          t0.lab_address ||
          ''
        ),
        lab_email: S(
          lab0.lab_pdfheader_email ||
          t0.lab_email ||
          ''
        ),
        lab_phone: S(
          lab0.lab_pdfheader_contact_no ||
          t0.lab_phone ||
          ''
        ),
      ...
      // ======== Row mappers (updated for richer data) ========
      ...
      // NEW: expanded STOP_DEFECTIVE
      const mapSTOP_DEF = (rec: any) => {
        const t = rec.testing || {};
        const d = rec.device || {};
        return {
          serial: S(d.serial_number) || N(d.serial_number),
          make: S(d.make),
          capacity: S(d.capacity || d.phase || ''),
          physical_condition_of_device: S(t.physical_condition_of_device),
          seal_status: S(t.seal_status),
      ...
      switch (rtype) {
        ...
        case 'STOP_DEFECTIVE': {
          ...
          await this.stopDefPdf.download(
            stopRows,
            headerForStopDef,
            {
              leftLogoUrl: commonHeaderBase.leftLogoUrl,
