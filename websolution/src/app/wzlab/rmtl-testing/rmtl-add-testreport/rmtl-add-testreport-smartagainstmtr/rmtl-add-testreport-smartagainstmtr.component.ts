...
interface DeviceRow {
  ...
  make: string;
  capacity: string;
  remark: string;                  // dropdown “Test Report Remark”
  test_result?: string;
  ...
  meter_body: string;
  other: string;                   // free note (separate from remark)
  is_burned: boolean;
...
export class RmtlAddTestreportSmartagainstmtrComponent implements OnInit {
  ...
  capacities: string[] = [];
  test_results: string[] = [];
  report_type: string = 'SMART AGAINST METER';
  ...
  approverId: number | null = null; // still sent to API if needed
  ...
  // ===================== Helpers =====================
    ...
    if (row.reading_before_test && row.reading_after_test && row.ref_start_reading && row.ref_end_reading) {
      row.error_percentage = Math.round(
        ...
        / (row.ref_end_reading - row.ref_start_reading)
      ) * 100 + 0.5,
      );
    }
    return 0;
  ...
  private buildPayloadForPreview(): any[] {
    ...
    return (this.batch.rows || [])
      ...
      .map(r => ({
        ...
        approver_id: this.approverId ?? null,          // keep if API expects it
  ...
  private async downloadPdfFromBatch(): Promise<void> {
    ...
    await this.smartPdf.download(snap.rows, snap.meta, {
  ...
  private doSubmitBatch(): void {
    ...
    this.api.postTestReports(this.payload).subscribe({
      next: async () => {
        ...
        try { await this.downloadPdfFromBatch(); } catch (e) { console.error('PDF generation failed:', e); }
        this.alertSuccess = 'Batch Report submitted successfully!';
        this.alertError = null;
        this.batch.rows = [this.emptyRow()];  // reset
        setTimeout(() => this.closeModal(), 1200);
      },
      error: (error) => {
        this.submitting = false;
        this.alertSuccess = null;
        this.alertError = 'Error submitting report. Please verify rows and try again.'+error;
