...
@Component({
  selector: 'app-rmtl-add-supply-othersource',
  templateUrl: './rmtl-add-supply-othersource.component.html',
  styleUrls: ['./rmtl-add-supply-othersource.component.css']
})
export class RmtlAddSupplyOthersourceComponent implements OnInit, AfterViewInit {
  ...
  onConfirmSubmit(): void {
    ...
    this.apiservice.createOtherSource(this.client).subscribe({
      next: (response) => {
        this.response_success = true;
        const name = response?.name || this.client.name;
        this.response_msg = `Other Source Location added successfully: ${name}`;
        this.loading = false;
      ...
      error: (error) => {
        console.error('Other Source Location creation failed:', error);
