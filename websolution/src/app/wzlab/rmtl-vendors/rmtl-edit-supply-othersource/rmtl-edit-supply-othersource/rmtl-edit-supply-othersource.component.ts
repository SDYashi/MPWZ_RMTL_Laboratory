...
@Component({
  selector: 'app-rmtl-edit-supply-othersource',
  templateUrl: './rmtl-edit-supply-othersource.component.html',
  styleUrls: ['./rmtl-edit-supply-othersource.component.css']
})
export class RmtlEditSupplyOthersourceComponent  implements OnInit, AfterViewInit {
  client: any = null;
  ...
  fetchClient(id: number): void {
    this.apiservices.getOtherSourcelocation(id).subscribe({
      next: (response) => { this.client = response; },
      error: (error) => {
        console.error('Failed to fetch other source client:', error);
        this.router.navigate(['/wzlab/supply-vendors/list-supply-othersource']);
      }
    });
  }
  ...
  onConfirmUpdate(): void {
    ...
    this.apiservices.updateOtherSource(this.client.id, this.client).subscribe({
      next: () => {
        this.response_success = true;
        this.response_msg = 'Other source Locations updated successfully';
        this.loading = false;
        this.alertModal.show();
      },
      error: (error) => {
        this.response_success = false;
        this.response_msg = error?.error?.message || error?.message || 'Failed to update other source Locations';
        this.loading = false;
        this.alertModal.show();
      }
    });
